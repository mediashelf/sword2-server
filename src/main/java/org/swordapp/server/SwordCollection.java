...
public class SwordCollection {
    ...
    private final List<String> multipartAccept = new ArrayList<String>();
    ...
    public SwordCollection() {
        abdera = new Abdera();
        collection = abdera.getFactory().newCollection();
    }
    ...
    public Collection getAbderaCollection() {
        Collection abderaCollection = (Collection) collection.clone();
        ...
        for (String mpa : multipartAccept) {
            Element element =
                    abdera.getFactory().newElement(UriRegistry.APP_ACCEPT);
        ...
        if (collectionPolicy != null) {
            abderaCollection.addSimpleExtension(
                    UriRegistry.SWORD_COLLECTION_POLICY, collectionPolicy);
        }

        // add the mediation
        abderaCollection.addSimpleExtension(UriRegistry.SWORD_MEDIATION,
                mediation ? "true" : "false");

        // add the treatment
        if (treatment != null) {
            abderaCollection.addSimpleExtension(UriRegistry.SWORD_TREATMENT,
                    treatment);
        }

        // add the acceptPackaging
        for (String ap : acceptPackaging) {
            abderaCollection.addSimpleExtension(
                    UriRegistry.SWORD_ACCEPT_PACKAGING, ap);
        }

        // add the sub service document IRI
        for (IRI ss : subServices) {
            abderaCollection.addSimpleExtension(UriRegistry.SWORD_SERVICE, ss
                    .toString());
        }

        // add the abstract
        if (dcAbstract != null) {
            abderaCollection.addSimpleExtension(UriRegistry.DC_ABSTRACT,
                    dcAbstract);
        }
    ...
    public void setAccept(String... mediaRanges) {
        collection.setAccept(mediaRanges);
    }
    ...
    public void addAccepts(String mediaRange) {
        collection.addAccepts(mediaRange);
    }

    public void addAccepts(String... mediaRanges) {
        collection.addAccepts(mediaRanges);
    }
    ...
    public void setMultipartAccept(String... mediaRanges) {
        ...
        multipartAccept.clear();
        multipartAccept.addAll(mrs);
    }

    public void addMultipartAccepts(String mediaRange) {
        multipartAccept.add(mediaRange);
    }
    ...
    public List<String> getMultipartAccept() {
        ...
        return rationaliseAccepts(accepts);
    }
