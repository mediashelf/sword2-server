...
public class ContainerAPI extends SwordAPIEndpoint {
    ...
    public ContainerAPI(ContainerManager cm, StatementManager sm,
    ...
    public void get(HttpServletRequest req, HttpServletResponse resp,
        ...
        try {
            ...
            String iri = getFullUrl(req);

            // the content negotiation may be for the deposit receipt, OR for
            // the Statement
            if (cm.isStatementRequest(iri, accept, auth, config)) {
                Statement statement =
                        sm.getStatement(iri, accept, auth, config);
                ...
                DepositReceipt receipt = cm.getEntry(iri, accept, auth, config);
                ...
                SimpleDateFormat sdf =
                        new SimpleDateFormat("EEE, dd MMM yyyy HH:mm:ss Z");
                Date lastModified =
                        receipt.getLastModified() != null ? receipt
                                .getLastModified() : new Date();
                resp.setHeader("Last-Modified", sdf.format(lastModified));
    ...
    public void put(HttpServletRequest req, HttpServletResponse resp)
        ...
        try {
            ...
            boolean inProgress = getInProgress(req);
            ...
            if (isMultipart) {
                ...
                receipt =
                        cm.replaceMetadataAndMediaResource(iri, deposit, auth,
                                config);
            } else if (isEntryOnly) {
                // check that we have the right content type
                if (!(contentType.startsWith("application/atom+xml") || contentType
                        .startsWith("application/atom+xml;type=entry"))) {
                ...
                addDepositPropertiesFromEntry(deposit, req);

                // now defer to the implementation layer
                receipt = cm.replaceMetadata(iri, deposit, auth, config);
            } else {
            ...
            if (location == null) {
                throw new SwordServerException(
                        "No Location found in Deposit Receipt; unable to send valid response");
            }
    ...
    public void post(HttpServletRequest req, HttpServletResponse resp)
        ...
        try {
            ...
            if (isEntryOnly) {
                addDepositPropertiesFromEntry(deposit, req);
                receipt = cm.addMetadata(iri, deposit, auth, config);
            } else if (headersOnly) {
                receipt = cm.useHeaders(iri, deposit, auth, config);
            } else {
    ...
    public void delete(HttpServletRequest req, HttpServletResponse resp)
        ...
        try {
            ...
            cm.deleteContainer(uri, auth, config);
