...
public class CollectionAPI extends SwordAPIEndpoint {
    ...
    public CollectionAPI(CollectionListManager clm,
            CollectionDepositManager cdm, SwordConfiguration config) {
        super(config);
        this.clm = clm;
        this.cdm = cdm;
    }
    ...
    public void get(HttpServletRequest req, HttpServletResponse resp)
        ...
        try {
            Feed feed =
                    clm.listCollectionContents(new IRI(getFullUrl(req)), auth,
                            config);
            ...
            if (feed == null) {
                // method not allowed
                resp.sendError(405,
                        "This server does not support listing collection contents");
                return;
            }

            // otherwise process and return
            this.addGenerator(feed, config);
    ...
    public void post(HttpServletRequest req, HttpServletResponse resp)
        ...
        try {
            ...
            String colUri = getFullUrl(req);
            receipt = cdm.createNew(colUri, deposit, auth, config);
            this.addGenerator(receipt, config);
