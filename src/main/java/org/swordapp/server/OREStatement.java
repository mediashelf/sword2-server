...
public class OREStatement extends Statement {
    ...
    public void writeTo(Writer out) throws IOException {
        ...
        model.setNsPrefix(UriRegistry.SWORD_PREFIX,
                UriRegistry.SWORD_TERMS_NAMESPACE);

        // create the resource map in the model
        Resource rem = model.createResource(remUri);
        rem.addProperty(RDF.type, model
                .createResource(UriRegistry.ORE_NAMESPACE + "ResourceMap"));

        // create the aggregation
        Resource agg = model.createResource(aggUri);
        agg.addProperty(RDF.type, model
                .createResource(UriRegistry.ORE_NAMESPACE + "Aggregation"));

        // add the aggregation to the resource (and vice versa)
        rem.addProperty(model.createProperty(UriRegistry.ORE_NAMESPACE +
                "describes"), agg);
        agg.addProperty(model.createProperty(UriRegistry.ORE_NAMESPACE +
                "isDescribedBy"), rem);

        // now go through and add all the ResourceParts as aggregated resources
        for (ResourcePart rp : resources) {
            Resource part = model.createResource(rp.getUri());
            part.addProperty(RDF.type, model
                    .createResource(UriRegistry.ORE_NAMESPACE +
                            "AggregatedResource"));
            agg.addProperty(model.createProperty(UriRegistry.ORE_NAMESPACE +
                    "aggregates"), part);
        }
        ...
        for (OriginalDeposit od : originalDeposits) {
            Resource deposit = model.createResource(od.getUri());
            deposit.addProperty(RDF.type, model
                    .createResource(UriRegistry.ORE_NAMESPACE +
                            "AggregatedResource"));
            if (od.getDepositedBy() != null) {
                deposit.addLiteral(model
                        .createProperty(UriRegistry.SWORD_DEPOSITED_BY), od
                        .getDepositedBy());
            }

            if (od.getDepositedOnBehalfOf() != null) {
                deposit.addLiteral(
                        model.createProperty(UriRegistry.SWORD_DEPOSITED_ON_BEHALF_OF),
            ...
            if (od.getDepositedOn() != null) {
                ...
                deposit.addLiteral(model
                        .createProperty(UriRegistry.SWORD_DEPOSITED_ON), sdf
                        .format(od.getDepositedOn()));
            }

            for (String packaging : od.getPackaging()) {
                deposit.addLiteral(
                        model.createProperty(UriRegistry.SWORD_PACKAGING
                                .toString()), packaging);
            }

            agg.addProperty(model.createProperty(UriRegistry.ORE_NAMESPACE +
                    "aggregates"), deposit);
            agg.addProperty(model
                    .createProperty(UriRegistry.SWORD_ORIGINAL_DEPOSIT),
                    deposit);
        }
        ...
        for (String state : states.keySet()) {
            ...
            if (states.get(state) != null) {
                s.addProperty(model
                        .createProperty(UriRegistry.SWORD_STATE_DESCRIPTION),
                        states.get(state));
            }
